       ...
       if (err) {
        ...
        if (!isNaN(Number(urlParam))) {
          ...
          dbOps.findOneByUrl(db, urlParam, "urlColl", function(docs) {
            ...
            if (docs.length > 0) {
              response.send("This shortened url already exists. Use the url" + <p>Hello, this is a p tag</p> + "http://matty22urlshortener.herokuapp.com/" + docs[0]._id + " to redirect to that location.");
            } else {
